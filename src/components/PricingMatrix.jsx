...
const plans = {
  ...
  satellite: [
    ...
    { name: 'Ushindi Satellite', speed: '1TB Priority', price: '14,072', features: ['Starlink Connectivity', 'Priority Support', 'Uninterrupted Operations', 'Enterprise Grade'], recommended: false },
  ]
};
    ...
    <section id="services" className="py-24 relative">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-400 max-w-2xl mx-auto">From high-speed fiber browsing to dedicated satellite enterprise solutions, our networks deliver unmatched reliability.</p>
        </div>
        ...
        <div className="flex justify-center mb-12">
          <div className="bg-[#111827] p-1 rounded-xl border border-slate-800 inline-flex">
            <button 
              ...
              className={`px-8 py-3 rounded-lg font-medium transition-all ${activeTab === 'fiber' ? 'bg-blue-600 text-white shadow-[0_0_15px_rgba(37,99,235,0.4)]' : 'text-slate-400 hover:text-white'}`}
            >
              Fiber Internet
            </button>
            <button 
              onClick={() => setActiveTab('satellite')}
              className={`px-8 py-3 rounded-lg font-medium transition-all ${activeTab === 'satellite' ? 'bg-blue-600 text-white shadow-[0_0_15px_rgba(37,99,235,0.4)]' : 'text-slate-400 hover:text-white'}`}
            >
              Satellite Internet
            </button>
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto justify-center">
          {plans[activeTab].map((plan, idx) => (
            <div key={idx} className={`relative bg-[#111827] rounded-2xl border ${plan.recommended ? 'border-blue-500 shadow-[0_0_30px_rgba(37,99,235,0.15)] md:-translate-y-4' : 'border-slate-800'} p-8 hover:-translate-y-1 transition-transform duration-300 flex flex-col`}>
              {plan.recommended && (
              ...
              <button className={`w-full py-3 mt-auto rounded-xl font-medium transition-colors ${plan.recommended ? 'bg-blue-600 hover:bg-blue-500 text-white' : 'bg-slate-800 hover:bg-slate-700 text-white'}`}>
                Get Connected
              </button>
            </div>
          ))}
